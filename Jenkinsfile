pipeline {
    ...
    stages {
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag  yash5090/brkg-bad:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/brkg-bad:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/brkg-bad:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/brkg-bad:latest'
                       sh 'docker-scout cves yash5090/brkg-bad:latest'
                       sh 'docker-scout recommendations yash5090/brkg-bad:latest'
                       sh 'docker-scout attestation yash5090/brkg-bad:latest'
                   }
                }   
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop brkg-bad | true"
                sh "docker rm brkg-bad | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name brkg-bad -p 3000:3000 yash5090/brkg-bad:latest' 
                //}
            }
        }
